...
def fetchAllAchievements (targetIDs, jobID = None):
    ...
    if jobID == None: 
        jobID = jobStart("Fetch achievements, all known players",0,"FetchAchievements.fetchAllAchievements",None)
...
def fetchIndividualsAchievements(ID):
    conn = connectToSource()
    cursor = conn.cursor()
    IDpieces = ID.split("-")
...
def manualTargetAchievements(targetID):
    fetchIndividualsAchievements(targetID)
